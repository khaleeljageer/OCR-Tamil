...
package com.jskaleel.vizhi_tamil.utils.tts_engine.tts.engine

import android.content.Context
import android.speech.tts.TextToSpeech
import com.jskaleel.vizhi_tamil.utils.tts_engine.utils.DEF_SPEECH_AND_PITCH
import com.jskaleel.vizhi_tamil.utils.tts_engine.utils.getErrorText
import com.jskaleel.vizhi_tamil.utils.tts_engine.utils.setListener
...
class TextToSpeechEngine private constructor() {
    ...
    companion object {
        ...
        val MAX_TEXT_LENGTH = TextToSpeech.getMaxSpeechInputLength()
    }
    ...
    fun initTTS(context: Context, message: String) {
        tts = TextToSpeech(context) {
            if (it == TextToSpeech.SUCCESS) {
                ...
                Timber.tag("Khaleel").d("MAX_TEXT_LENGTH: $MAX_TEXT_LENGTH")
                if (message.length > MAX_TEXT_LENGTH) {
                    message.chunked(MAX_TEXT_LENGTH).forEach { chunk ->
                        speak(chunk, TextToSpeech.QUEUE_ADD)
                    }
                } else {
                    speak(message)
                }
